...
`apps/desktop/package.json` é a versão do desktop. PRs que alteram `apps/desktop` precisam avançá-la e sincronizar `package-lock.json`; a Action verifica isso. Uma tag `vX.Y.Z` que corresponde a ela inicia a Action Windows: testes, build nativo, instalador NSIS por usuário, ZIP portátil standalone com checksum SHA-256, manifesto e assinatura Ed25519. O resultado é uma **release em rascunho**. Para usar o standalone, extraia o ZIP inteiro e execute `Lumen Desktop.exe` dentro da pasta `Lumen Desktop`; ele não precisa de instalação. Publique a release depois de testar manualmente vídeo, áudio, instalação e atualização em Windows 11. Só releases publicadas são consultadas pelo app.
